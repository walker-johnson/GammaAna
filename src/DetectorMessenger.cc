...
:G4UImessenger(), 
 fDetector(Det), fTestemDir(0), fDetDir(0), fMaterCmd(0), fSizeCmd(0), fGammaCmd(0),
 fIsotopeCmd(0)
{ 
  ...
  fGammaCmd = new G4UIcmdWithADoubleAndUnit("/testhadr/det/setGammaDetPos",this);
  fGammaCmd->SetGuidance("Set the x position of the gamma detector");
  fGammaCmd->SetParameterName("X",false);
  fGammaCmd->SetRange("15.>=X>=-15.");
  fGammaCmd->SetUnitCategory("Length");
  fGammaCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
...
void DetectorMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  ...
  if( command == fGammaCmd )
    { fDetector->SetGammaCounterX(fGammaCmd->GetNewDoubleValue(newValue));}
