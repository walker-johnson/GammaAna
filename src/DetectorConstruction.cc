 ...
 worldP(0), worldL(0), fMaterial(0), fDetectorMessenger(0)
{
  ...
  fGap = 10*cm;
  fDDHead_x = 0*cm;
  fDDHead_y = -fChamber_y/2 + fPoly_y/2 + 2.5*cm  + 37.5/4*cm;
...
G4VPhysicalVolume* DetectorConstruction::ConstructVolumes()
{
  ...
  */
  ...
  //Now add a lead box to hold the gamma detector


  double gammaBoxX = 12*cm;
  double gammaBoxY = 12*cm;
  double gammaBoxZ = 12*cm;
  double leadThickness = 2.5*cm;
  
  G4Material* lead =
     G4NistManager::Instance()->FindOrBuildMaterial("G4_Pb");

  
  G4Box* gammaDetBoxS = new G4Box("gammaDetBox",
				  gammaBoxX/2, gammaBoxY/2, gammaBoxZ/2);
  
  gammaDetBoxL = new G4LogicalVolume(gammaDetBoxS,
				     lead,
				     "gammaDetBox");

  gammaDetBoxP = new G4PVPlacement(0,
				   G4ThreeVector(fGammaCounterX,
						 -fChamber_y/2 + fPoly_y + 2.5*cm + gammaBoxY/2,
						 -fChamber_z/2 + gammaBoxZ/2),
				   gammaDetBoxL,
				   "gammaDetBox",
				   chamberL,
				   false,
  ...
  G4Box* gammaDetS = new G4Box("gammaDet",
			       gammaBoxX/2-leadThickness,
			       gammaBoxY/2-leadThickness/2,
			       gammaBoxZ/2-leadThickness);

  gammaDetL = new G4LogicalVolume(gammaDetS,
				  fMaterial,
				  "gammaDet");

  gammaDetP = new G4PVPlacement(0,
				G4ThreeVector(0, -leadThickness/2, 0),
				gammaDetL,
				"gammaDet",
				gammaDetBoxL,
				false,
...
void DetectorConstruction::SetGammaCounterX(G4double x){
  fGammaCounterX = x;
  G4RunManager::GetRunManager()->ReinitializeGeometry();
}
